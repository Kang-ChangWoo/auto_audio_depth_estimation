...
Baseline **0.4434 / 1.5907 / 0.5236** → champion **E121 0.3424 / 1.4708 / 0.5806** (comp **2.088**, commit `647bfc7`): **ABS_REL −23%, RMSE −7.5%, d1 +5.7 pts**. Champion mean over 20+ confirmation reruns ≈ **2.099±0.008**; best draws reach 2.087. Only Δ clearing ~0.008 (and ideally replicated by a confirmation rerun) is real.

**E116→E121 recent wins:** E116 dropped the vestigial encoder tail (24.47→7.69M params, −69%, provably equivalent). E121 **extended the geometry-aware attention axis into the ray↔audio cross-attention** — a `GeoCrossBlock` that adds a learned per-head angular-distance bias between the ray-query direction and the audio-token direction (both on the same 16×32 ERP grid) so each coarse ray reads the geometrically-relevant audio tokens. Compute-neutral; confirmed by rerun E122 (2-draw mean 2.0892 vs champion mean 2.099). Geometry-aware ray-conditioned reasoning remains THE productive axis — it now spans coarse self-attn (rsa16b), local self-attn (lsa32/64), AND coarse cross-attn (cr16).
